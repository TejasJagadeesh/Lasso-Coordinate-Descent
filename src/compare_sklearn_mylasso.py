...
class CompareSklearnMylasso:
    ...
    def __init__(self):
        ...
        The L1-Regularization parameter lambda is set to 1.0 by default.
    ...
    def fit_sklearn_model(self, X_train, y_train, intercept):
        """
        Fits Sklearn LassoCV model on the specified train and test dataset.
        ...
        intercept: boolean
        This is boolean indicator to fit the intercept.
        
        Returns:
        --------
        sklearn_model: Sklearn model
        The model fit by Sklearn LassoCV is returned.
        
        """
        model = LassoCV(cv = 10, n_alphas = 1000, fit_intercept = intercept, max_iter = 1000)
...
if __name__=='__main__':
    ...
    """
    dataset_type = input("Enter Y for comparison using Real Dataset and N for Simulated Dataset: ")
    if dataset_type not in ['Y','N']:
        print("Please enter Y/N")
    else:
        algo = CompareSklearnMylasso()
        if dataset_type == 'N':
            X_train, y_train, X_test, y_test = algo.create_simulated_data()
            intercept = True
            ...
            X_train, y_train, X_test, y_test = algo.preprocess_real_data()
            intercept = False
        ...
        print("Training using Sklearn started ...",end='')
        sklearn_model = algo.fit_sklearn_model(X_train, y_train, intercept)
        print("Complete!!\n")
        print("Training using My Lasso Algorithm started ...",end='')
        betas_cyc = algo.cycliccoorddescent(X_train, y_train)
        betas_rnd = algo.randcoorddescent(X_train, y_train)
        print("Complete!!\n")
        print("Here is the comparison of the coefficients:\n")
        print(pd.DataFrame({'Sklearn':sklearn_model.coef_,'Cyclic':betas_cyc[-1],'Random':betas_rnd[-1]}))